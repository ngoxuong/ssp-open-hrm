...
--------------------------------------------------------------------------- */
  function checkDate(_dateControl, _format){
   ...
   if ((_numMonth==1)||(_numMonth==3)||(_numMonth==5)||(_numMonth==7)||(_numMonth==8)||(_numMonth==10)||(_numMonth==12)){
	...
	if(_format) _dateControl.value=_day+"/"+_month+"/"+_year;
      return "TRUE";
...
/*-- Ham tinh khoang cach giua 2 ngay voi tham so truyen vao la date1, date2(dd/mm/yyyy). Voi date2 phai sau date1
	Tra ve so int khoang cach giua 2 ngay
	-------------------------------
*/
function distanceDate1Date2(date1, date2){
	var day1 = getDateToInt(date1);
    var month1 = getMonthToInt(date1);
	var year1 = getYearToInt(date1);
    var day2 = getDateToInt(date2);
    var month2 = getMonthToInt(date2);
    var year2 = getYearToInt(date2);
    var flag = 0;	 
	if(month1<8){
		if(month1%2==1)
			flag = 31;
	...
	var khoangcach = year*365 + month*flag + day + 1;
	return khoangcach;
}