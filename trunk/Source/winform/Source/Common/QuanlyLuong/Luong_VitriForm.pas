...
const
  ...
  S_KEY_FIELD='MA_VITRI';
type
  TfrmLuong_Vitri = class(TfrmBase)
    ...
    qryDataMA_VITRI: TWideStringField;
    qryDataPOSITION_NAME: TWideStringField;
...
var
  frmLuong_Vitri: TfrmLuong_Vitri;
...
constructor TfrmLuong_Vitri.Create(AOwner: TComponent; PA_MA: integer;
...
procedure TfrmLuong_Vitri.acPhatsinhDulieuExecute(Sender: TObject);
begin
  inherited;
  qryExecute.SQL.Clear;
  qryExecute.SQL.Text := 'insert into '+S_PREFIX_TABLE+ F_PA_KYHIEU + ' ('+S_KEY_FIELD+') ' +
    ' select POSITION_NO from HR_POSITION where POSITION_NO not in ' +
    ' (select '+S_KEY_FIELD+' from '+S_PREFIX_TABLE+F_PA_KYHIEU + ')';
  qryExecute.ExecSQL;
  acXemdulieu.Execute;
end;

procedure TfrmLuong_Vitri.acXemdulieuExecute(Sender: TObject);
begin
  inherited;
...
procedure TfrmLuong_Vitri.acInitQueryExecute(Sender: TObject);
var
  i: integer;
...
begin
  ...
  qryData.SQL.Clear;
  qryData.SQL.Text := 'SELECT '+S_KEY_FIELD+', TITLE_NAME||''-''||DEPT_NAME POSITION_NAME, P_POSITION_NO ';
  ...
  begin
    ...
    qryData.SQL.Text:=qryData.SQL.Text+
         ...
         ' left JOIN HR_POSITION on POSITION_NO='+S_KEY_FIELD+
...
procedure TfrmLuong_Vitri.acCapnhatDulieuExecute(Sender: TObject);
begin
  inherited;
  ...
  begin
    ...
    '  from '+S_PREFIX_TABLE+F_PA_KYHIEU +
    '  where '+S_PREFIX_TABLE+F_PA_KYHIEU+'.ma_vitri='+S_PREFIX_WAGE_TABLE+F_PA_KYHIEU+'.ma_chucvu||''-''||'+S_PREFIX_WAGE_TABLE+F_PA_KYHIEU+'.ma_donvi)'+
...
procedure TfrmLuong_Vitri.acCapnhatDulieuUpdate(Sender: TObject);
begin
  inherited;
  acCapnhatDulieu.Enabled:=not qryCongthuc.IsEmpty;
end;

procedure TfrmLuong_Vitri.FormClose(Sender: TObject;
