...
uses
  ...
  ElBtnCtl, ElPopBtn, dxEdLib, dxEditor, DateTimeFrame, DB, IBODataset,
  ImgList, ActnList;

type
  TfrmBAOHIEM_DSPHSK_SauTS = class(TfrmEditBase)
...
var
  frmBAOHIEM_DSPHSK_SauTS: TfrmBAOHIEM_DSPHSK_SauTS;

implementation

uses PopupMainForm, MainDM, SSP_Library;

{$R *.dfm}

procedure TfrmBAOHIEM_DSPHSK_SauTS.FormCreate(Sender: TObject);
begin
  dximOption.Tag := 1;
  ...
  RegisterLookupControl(gridChiTietWHT_NAME, 'LOAI_DOITUONG','wht_name',
    'select wht_no, wht_name from hr_work_hour_type where la_ndsphsk_sau_ts =1;');
  frameDateTime1.SetPeriod('NAM_NAY');
  acXemDS.Execute;
end;

procedure TfrmBAOHIEM_DSPHSK_SauTS.acXemDSExecute(Sender: TObject);
begin
  inherited;
...
procedure TfrmBAOHIEM_DSPHSK_SauTS.dxSelectDeptInitPopup(Sender: TObject);
begin
  inherited;
...
procedure TfrmBAOHIEM_DSPHSK_SauTS.dxSelectDeptCloseUp(Sender: TObject;
...
procedure TfrmBAOHIEM_DSPHSK_SauTS.dxSelectDeptChange(Sender: TObject);
begin
  inherited;
...
procedure TfrmBAOHIEM_DSPHSK_SauTS.FormClose(Sender: TObject;
...
procedure TfrmBAOHIEM_DSPHSK_SauTS.gridChiTietTEN_VANBAN_DINHKEMButtonClick(
...
procedure TfrmBAOHIEM_DSPHSK_SauTS.qryChiTietNGHI_TUNGAYChange(
  Sender: TField);
begin
  inherited;
...
procedure TfrmBAOHIEM_DSPHSK_SauTS.qryChiTietNGHI_DENNGAYChange(
  Sender: TField);
begin
  inherited;
  if ((not qryChiTietNGHI_TUNGAY.IsNull) and (not qryChiTietNGHI_DENNGAY.IsNull)) then
...
procedure TfrmBAOHIEM_DSPHSK_SauTS.qryChiTietBeforePost(DataSet: TDataSet);
begin
  inherited;
...
procedure TfrmBAOHIEM_DSPHSK_SauTS.acDocDLChamCongExecute(Sender: TObject);
begin
  inherited;
  try
    qryExecute.SQL.Clear;
    qryExecute.SQL.Text := 'EXECUTE PROCEDURE HR_BAOHIEM_CAPNHAT_DSPHSK_SAUTS(:XEM_THEO_DONVI,:XEM_CAPDUOI,:XEM_TUNGAY,:XEM_DENNGAY,:USER_NAME);';
...
procedure TfrmBAOHIEM_DSPHSK_SauTS.acDocDLChamCongUpdate(Sender: TObject);
begin
  inherited;
