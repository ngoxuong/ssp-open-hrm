...
uses
  ...
  Series, DBChart, TeeComma, StdCtrls, TeePyramid;

type
  TfrmTrainingBudget = class(TfrmBase)
    ...
    dsTrainCost: TDataSource;
    memTrainCost: TdxMemData;
    memTrainCostMONTH: TIntegerField;
    memTrainCostYEAR: TIntegerField;
    memTrainCostMONTH_YEAR: TWideStringField;
    acInitTrainCost: TAction;
    chartTrainCost: TDBChart;
    Series1: TBarSeries;
    ...
    gridDataALLOWANCE: TdxDBGridColumn;
    gridDataCOST: TdxDBGridColumn;
    Label1: TLabel;
    memTrainCostCOST: TFloatField;
    memTrainResultCOST: TFloatField;
    BarSeries1: THorizBarSeries;
    memTrainContentCOST: TFloatField;
    procedure FormCreate(Sender: TObject);
    procedure acXemdulieuExecute(Sender: TObject);
    procedure FormClose(Sender: TObject; var Action: TCloseAction);
    procedure acInitTrainCostExecute(Sender: TObject);
...
var
  frmTrainingBudget: TfrmTrainingBudget;
...
procedure TfrmTrainingBudget.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TfrmTrainingBudget.acXemdulieuExecute(Sender: TObject);
begin
  inherited;
...
procedure TfrmTrainingBudget.FormClose(Sender: TObject;
...
procedure TfrmTrainingBudget.acInitTrainCostExecute(Sender: TObject);
var i: integer;
...
begin
  ...
  qryData.DisableControls;
  memTrainCost.DisableControls;
  if memTrainCost.Active then memTrainCost.Close;
  memTrainCost.Open;
  //==========================================
  ...
  begin
    memTrainCost.Append;
    memTrainCostMONTH.Value := (i mod 12) + 1;
    memTrainCostYEAR.Value := (i div 12);
    if (memTrainCostMONTH.Value = 1) or
      (i = first_my) or (i = last_my)
      then
      memTrainCostMONTH_YEAR.Value := memTrainCostMONTH.AsString + '/' + memTrainCostYEAR.AsString
    else
      memTrainCostMONTH_YEAR.Value := memTrainCostMONTH.AsString;
    memTrainCostCOST.Value := 0;
    memTrainCost.Post;
  end;
  //============================================
  ...
  begin
    ...
    begin
      ...
      if not memTrainCost.Locate('MONTH;YEAR', VarArrayOf([m, y]), []) then
        ShowMessage('Error : can not locate ' + IntToStr(m) + '/' + IntToStr(y))
      else
      begin
        memTrainCost.Edit;
        memTrainCostCOST.Value := memTrainCostCOST.Value + StrToFloatDef(VarToStr(node.Values[gridDataALLOWANCE.Index]),0);
        memTrainCost.Post;
      end;
    end;
  end;
  memTrainCost.EnableControls;
  qryData.EnableControls;
  chartTrainCost.RefreshData;
  Screen.Cursor := crDefault;
end;

procedure TfrmTrainingBudget.pageDataChanging(Sender: TObject;
  NewPage: TElTabSheet; var AllowChange: Boolean);
begin
  inherited;
  if NewPage = tabSoluong then
    acInitTrainCost.Execute
  else
...
procedure TfrmTrainingBudget.acInitTrainResultExecute(Sender: TObject);
var i: integer;
  node: TdxTreeListNode;
  temp: Variant;
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        memTrainResult.FieldByName('COST').Value :=node.Values[gridDataALLOWANCE.Index];
      ...
      begin
        memTrainResult.Edit;
        memTrainResultCOST.Value :=memTrainResultCOST.Value +StrToFloatDef(VarToStr(node.Values[gridDataALLOWANCE.Index]),0);
        memTrainResult.Post;
      end;
    end;
  end;
...
procedure TfrmTrainingBudget.acInitTrainContentExecute(Sender: TObject);
var i: integer;
  node: TdxTreeListNode;
  temp: Variant;
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        memTrainContentCOST.Value := StrToFloatDef(VarToStr(node.Values[gridDataALLOWANCE.Index]),0);
      ...
      begin
        ...
        memTrainContentCOST.Value :=memTrainContentCOST.Value+StrToFloatDef(VarToStr(node.Values[gridDataALLOWANCE.Index]),0);
        memTrainContent.Post;
      end;
    end;
  end;
