...
begin
  ...
  try
    ...
    dmMain.DefOnBeforePrint(false, ContractDM.qryContract, ContractDM);
...
begin
  ...
  try
    if PrepareReport = false then exit;
    dmMain.DefOnBeforePrint(true, ContractDM.qryContract, ContractDM);
...
procedure TfrmContractManager.acQDDeleteUpdate(Sender: TObject);
begin
  inherited;
  acQDDelete.Enabled:=not( ContractDM.qryContract.IsEmpty or
       (ContractDM.qryContract.State in [dsInsert,dsEdit]));
end;

procedure TfrmContractManager.acQDDeleteExecute(Sender: TObject);
var
  i : integer;
begin
  inherited;
  //Edit by THUYPTP - 29/4/2009
  //dmMain.DeleteExecute(ContractDM.qryContract);
  if (ContractDM.qryContract.State in [dsInsert, dsEdit]) then
    exit;
  if ContractDM.qryContract.IsEmpty then
    exit;
  if ShowMessageUnicode(9) = mrYes then
  begin
    for i:=0 to gridContractList.SelectedCount -1 do
    begin
      try
        ContractDM.qryContract.Delete;
        if not ContractDM.qryContract.IB_Transaction.AutoCommit then
          ContractDM.qryContract.IB_Transaction.CommitRetaining;
      except
      end;
    END;
    acViewReport.Execute;
