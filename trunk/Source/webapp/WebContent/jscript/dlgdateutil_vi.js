...
function show_cal_dlg(textfieldname, imagename, valuedate){
	...
	dlgCalWin = window.open("../common/dlgdate.jsp?tb=" + textfieldname + "&im=" + imagename + "&date=" + valuedate, "_dlg", "status,titilebar=no,resizeable,height=205,width=173,top=" + _top + ",left=" + _left);
...
function funcSetDate(day, month, year, TextBoxName){
   ...
   opener.document.frmMain[date.textName].value = stDay + '/' + stMonth + '/' + year
   window.close();
}