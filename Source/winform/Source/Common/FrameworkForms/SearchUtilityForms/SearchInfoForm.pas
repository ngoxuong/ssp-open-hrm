...
uses
  ...
  ActnList, IB_Components, IB_Events, ComCtrls, StdCtrls, ElCLabel, ElLabel,
    DateUtils;
...
type
  TfrmSearhUtils = class(TForm)
    ...
    dxSpinVal1: TdxSpinEdit;
    dxlcSpinVal1: TdxLayoutItem;
    dxlcSpinVal2: TdxLayoutItem;
    dxSpinVal2: TdxSpinEdit;
    dxlcSpinVal3: TdxLayoutItem;
    dxSpinVal3: TdxSpinEdit;
    dxlcSpinVal4: TdxLayoutItem;
    dxSpinVal4: TdxSpinEdit;
    dxlcSpinVal5: TdxLayoutItem;
    dxSpinVal5: TdxSpinEdit;
    ...
    procedure dxSpinVal1Change(Sender: TObject);
  private
...
uses SSP_Library, SSP_Menus, MainUnit, Math, MainDM, EmployeeListForm,
  DecisionDataModule, ContractDataModule, DeptListForm, DecisionManageForm,
  ContractManageForm;
{$R *.dfm}
...
begin
  result := true;
  try
    qryListItemvalue.SQL.Clear;
    qryListItemvalue.SQL.Text := qryListItemSQL_GET_VALUE.AsString;
    dmMain.DefOnBeforeOpen(qryListItemValue);
...
begin
  ...
  qrySearchObjSUBSYSTEM_ID.Value := sysConfig.SubSystemID;
end;
...
begin
  ...
  begin
    ...
    begin
      if qrySQLPrepare.Fields.FindField(qrySearchItemFIELD_ID.AsString) = nil
        then
...
begin
  ...
  if dxItem1.Tag = 2 then
  begin
    dxlcExVal1.Visible := false;
    dxlcLikeVal1.Visible := false;
    dxlcDateVal1.Visible := false;
    dxlcSpinVal1.Visible := true;
  end
  else
  begin
  ...
  if dxItem2.Tag = 2 then
  begin
    dxlcExVal2.Visible := false;
    dxlcLikeVal2.Visible := false;
    dxlcDateVal2.Visible := false;
    dxlcSpinVal2.Visible := true;
  end
  else
  begin
  ...
  if dxItem3.Tag = 2 then
  begin
    dxlcExVal3.Visible := false;
    dxlcLikeVal3.Visible := false;
    dxlcDateVal3.Visible := false;
    dxlcSpinVal3.Visible := true;
  end
  else
  begin
  ...
  if dxItem4.Tag = 2 then
  begin
    dxlcExVal4.Visible := false;
    dxlcLikeVal4.Visible := false;
    dxlcDateVal4.Visible := false;
    dxlcSpinVal4.Visible := true;
  end
  else
  begin
  ...
  if dxItem5.Tag = 2 then
  begin
    dxlcExVal5.Visible := false;
    dxlcLikeVal5.Visible := false;
    dxlcDateVal5.Visible := false;
    dxlcSpinVal5.Visible := true;
  end
  else
  begin
...
var
  ...
  param_count: integer;
begin
  if (dxItem1.Visible = true) and (dxItem1.Text = '') then
  begin
    ShowMessageUnicode(115);
    exit;
  end;
  if (dxExval1.Visible = true) and (dxExval1.Text = '') then
  begin
    ShowMessageUnicode(115);
    exit;
  end;
  if (dxItem2.Visible = true) and (dxItem2.Text = '') then
  begin
    ShowMessageUnicode(115);
    exit;
  end;
  if (dxExval2.Visible = true) and (dxExval2.Text = '') then
  begin
    ShowMessageUnicode(115);
    exit;
  end;
  if (dxItem3.Visible = true) and (dxItem3.Text = '') then
  begin
    ShowMessageUnicode(115);
    exit;
  end;
  if (dxExval3.Visible = true) and (dxExval3.Text = '') then
  begin
    ShowMessageUnicode(115);
    exit;
  end;
  if (dxItem4.Visible = true) and (dxItem4.Text = '') then
  begin
    ShowMessageUnicode(115);
    exit;
  end;
  if (dxExval4.Visible = true) and (dxExval4.Text = '') then
  begin
    ShowMessageUnicode(115);
    exit;
  end;
  if (dxItem5.Visible = true) and (dxItem5.Text = '') then
  begin
    ShowMessageUnicode(115);
    exit;
  end;
  if (dxExval5.Visible = true) and (dxExval5.Text = '') then
  begin
    ShowMessageUnicode(115);
    exit;
  end;
  ...
  begin
    if (dxOp1.Text <> 'like') and (dxOp1.Text <> 'is null') then
      ...
      qryInfo.SQLWhere.Text := ' WHERE (' + PARAM_ID[1] + ' like ''%''|| :' +
        PARAM_ID[1] + '_1 ||''%'')';
      //ShowMessage(qryInfo.SQLWhere.Text);
      end
    else
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          if (Assigned(qryInfo.FindParam(PARAM_ID[i] + '_' + IntToStr(i)))) then // fix TH is null
            qryInfo.ParamByName(PARAM_ID[i] + '_' + IntToStr(i)).Clear
        end
        else
        begin
          if PARAM_TYPE[i] = 0 then //kieu ngay
          begin
            qryInfo.ParamByName(PARAM_ID[i] + '_' + IntToStr(i)).Value :=
          ...
          begin
            ...
            if (Assigned(qryInfo.FindParam(PARAM_ID[i] + '_' + IntToStr(i)))) then // fix TH is null
            begin
              qryInfo.ParamByName(PARAM_ID[i] + '_' + IntToStr(i)).Value := p_val;
            end;  
          end;
        end;
      end;
...
begin
  ...
  begin
    ...
    begin
      ShowMDIForm(TfrmContractManager, frmContractManager);
...
procedure TfrmSearhUtils.dxSpinVal1Change(Sender: TObject);
begin
  PARAM_VALUE[1] := dxSpinVal1.Value;
end;
