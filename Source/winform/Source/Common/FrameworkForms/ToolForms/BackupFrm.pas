...
const
  // string constants - no locale specific - used in settings file
...
procedure TfrmBackup.SaveSettingsFile(Sender: TObject);
begin
  SaveSettingsToFile(FLoadedFile, false);
end;

procedure TfrmBackup.SaveSettingsFileAs(Sender: TObject);
begin
  SaveSettingsToFile('', true);
end;

procedure TfrmBackup.DBakLogEvent(Sender: TDBak_Component;
  AEntry: TDBak_LogEntry; ADescrip: string; ALongMessage: string;
  var DoDefault, AProceed: Boolean);
begin
  FTmpResults.Add(FormatDateTime('hh:nn:ss', SysUtils.Now) +
    ' ' + ADescrip);
...
procedure TfrmBackup.DBakNotifyEvent(Sender: TDBak_Component;
  AEvent: TDBak_NotifyEventType; var AProceed: Boolean);
begin
  case AEvent of
    neBeginOperation:
      begin
        OperationDisplayTxt.Caption :=
          DBakOperationToText(Sender.CurrentOperation);
...
procedure TfrmBackup.DBakSaveScript(Sender: TDBak_Component; AScript,
  AProbs: TStrings; var AProceed: Boolean);
var
  i: integer;
begin
  // Simply want to add the list of possible problems to the results display
  for i := 0 to AProbs.Count - 1 do
    FTmpResults.Add('         ' + AProbs.Strings[i]);
end;

procedure TfrmBackup.DBakShowProgress(Sender: TDBak_Component;
...
procedure TfrmBackup.ShowConfigurationPg;
begin
...
procedure TfrmBackup.ShowRunningPg;
begin
...
procedure TfrmBackup.ShowResultsPg;
begin
  case DBak.CompletionCode of
    ccSuccess: ElFormCaption1.Texts[2].Caption :=
      Utf8Decode('Kết thúc thao tác thành công!');
    ccWarnings: ElFormCaption1.Texts[2].Caption :=
      Utf8Decode('Kết thúc thao tác có cảnh báo!');
    ccErrors: ElFormCaption1.Texts[2].Caption := Utf8Decode('Thao tác có lỗi!');
    ccUserAbort: ElFormCaption1.Texts[2].Caption := Utf8Decode('Thao tác bị bỏ qua!');
    ccFatal: ElFormCaption1.Texts[2].Caption := Utf8Decode('Lỗi hệ thống!');
  end;
...
procedure TfrmBackup.LoadSettingsFromFile(AFile: string);
begin
  // Dont do anything if file has not changed
...
function TfrmBackup.SaveSettingsToFile(AFile: string; DoPrompt: boolean):
  boolean;
begin
  Result := true; // save not aborted
...
procedure TfrmBackup.StartDBak;
begin
  FStopping := false;
  ...
  try
    ...
    FEndDT := SysUtils.Now;
    FTmpResults.Add(
      Format(STR_DBak_RunTime, [
      FormatDateTime('hh.nn.ss', (FEndDT - FStartDT))])
        );
    ShowResultsPg;
  end;
end;

procedure TfrmBackup.StopDBak;
begin
  FStopping := true;
end;

procedure TfrmBackup.DrawAnimation(Idx: TImageIndex);
begin
...
procedure TfrmBackup.elbtnStartClick(Sender: TObject);
var
  gen: smallint;
begin
...
procedure TfrmBackup.elbtnCloseClick(Sender: TObject);
begin
  ModalResult := mrOK;
end;

procedure TfrmBackup.elbtnStopClick(Sender: TObject);
begin
  StopDBak
end;
{-----------------------------------------------------------------------------
  Procedure: TfrmBackup.Create
  Description
  Author:    quantd
  Date:      28-Feb-2005
  Arguments:
          AOwner: TComponent;
          AOperation: smallint  0: backup 1:restore
  Result:    None
-----------------------------------------------------------------------------}

constructor TfrmBackup.Create(AOwner: TComponent; AOperation: smallint);
var FIleName:String;
begin
  ...
  begin
    dxpiedtUserProtocol.ItemIndex := 2;
    dxpiedtBackupProtocol.ItemIndex := 2;
    dxedtUserDB.Text := dmMain.connMain.DatabaseName;
  end;
  //Anh Quan
  ...
  begin
    ...
    CodeOptions := [coDefineShadows, coSetGeneratorVal, coAlwaysCharSet,
      coKeepCustomIndexNames];
    ProgressUpdates := 250;
    ProgressSleep := -1;
  end;
...
procedure TfrmBackup.dxedtUserDBButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
  if DBakC.SelectedSet.Operation=opBackup then
    FileDlgBackup.Options:=FileDlgBackup.Options+[ofFileMustExist]
  else
    FileDlgBackup.Options:=FileDlgBackup.Options-[ofFileMustExist];
...
procedure TfrmBackup.dxedtBackupDBButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
  if DBakC.SelectedSet.Operation=opBackup then
    FileDlgRestore.Options:=FileDlgBackup.Options-[ofFileMustExist]
  else
    FileDlgRestore.Options:=FileDlgBackup.Options+[ofFileMustExist];
...
procedure TfrmBackup.dxedtUserServerChange(Sender: TObject);
begin
  if dxedtUserServer.Text = '' then
    dxpiedtUserProtocol.ItemIndex := 0;
end;

procedure TfrmBackup.dxedtBackupServerChange(Sender: TObject);
begin
