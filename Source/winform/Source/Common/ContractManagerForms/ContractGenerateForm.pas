...
type
  TfrmContractGenerate = class(TfrmBase)
    ...
    qryTemplateWAGE_SCALE: TWideStringField;
    qryTemplateWAGE_LEVEL: TIntegerField;
    qryTemplateWAGE_COEFF: TIBOFloatField;
    qryTemplateAPPLY_MONTH: TSmallintField;
    qryTemplateAPPLY_YEAR: TIntegerField;
    qryTemplateAPPLY_PERIOD: TSmallintField;
    qryTemplateCAREER_NAME: TWideStringField;
    ...
    qryWageStandardDetail: TIBOQuery;
    ElPopupButton2: TElPopupButton;
    dxLayoutControl4Item8: TdxLayoutItem;
    acXoaViTriBoNhiem: TAction;
    acXoaNgachLuong: TAction;
    ElPopupButton3: TElPopupButton;
    dxLayoutControl6Group4: TdxLayoutGroup;
    dxLayoutControl6Item7: TdxLayoutItem;
    dxDBMemo2: TdxDBMemo;
    dxLayoutControl1Item9: TdxLayoutItem;
    dxXoaThamSo: TdxBarButton;
    acXoaThamSo: TAction;
    ...
    procedure dxDBPopupEdit5CloseUp(Sender: TObject; var Text: WideString;
      var Accept: Boolean);
    procedure acXoaViTriBoNhiemExecute(Sender: TObject);
    procedure acXoaNgachLuongExecute(Sender: TObject);
    procedure acXoaThamSoExecute(Sender: TObject);
  private
        ...
        begin
          if memListSELECTED.Value = 1 then
            ...
            begin
              ...
              if memListGENDER.IsNull then
                memEmployeeGENDER.Clear
              else
                memEmployeeGENDER.Value:=memListGENDER.Value;
              if memListBIRTH_DATE.IsNull then
                memEmployeeBIRTH_DATE.Clear
              else
                memEmployeeBIRTH_DATE.Value:=memListBIRTH_DATE.Value;
              if memListEDU_LEVEL_NAME.IsNull then
                memEmployeeEDU_LEVEL_NAME.Clear
              else
                memEmployeeEDU_LEVEL_NAME.Value:=memListEDU_LEVEL_NAME.Value;
              if memListCAREER_NAME.IsNull then
                memEmployeeCAREER_NAME.Clear
              else
...
begin
  inherited;
  if (qryTemplate.State in [dsEdit,dsInsert]) then
    qryTemplate.Post;
...
begin
  ...
  begin
    ...
    try
      ...
      qryCreateContract.ParamByName('CONTRACT_NOTE').Value:=qryTemplateCONTRACT_NOTE.Value;
    ...
    // phat sinh bo nhiem neu co
      ...
      begin
        ...
        qryExecute.SQL.Text := 'select hr_assignment.employee_no from hr_assignment ' +
                      'where hr_assignment.employee_no = :EMLOYEE_NO ' +
                      'and hr_assignment.position_no = :POSITION_NO ' +
                      'and hr_assignment.dismiss_date is null';
        qryExecute.ParamByName('EMLOYEE_NO').Value := memEmployeeEMPLOYEE_NO.Value;
        qryExecute.ParamByName('POSITION_NO').Value := qryTemplatePOSITION_NO.Value;
        qryExecute.Open;
    ...
    // phat sinh he so luong di kem
      ...
      //Phat sinh he so luong co ban nha nuoc neu co
      if not (qryTemplateWAGE_SCALE.IsNull) then
      begin
        try
          if qryWageStandardChange.Active then qryWageStandardChange.Close;
          qryWageStandardChange.ParamByName('CONTRACT_NO').Value:=Contract_NO;
          qryWageStandardChange.ParamByName('CONTRACT_DATE').Value:=qryTemplateSTARTED_DATE.Value;
          qryWageStandardChange.ParamByName('CONTRACT_NAME').Value:=qryTemplateCONTRACT_NAME.Value;
          qryWageStandardChange.Open;  

          qryWageStandardDetail.ParamByName('CHANGE_NO').Value := qryWageStandardChangeCHANGE_NO.Value;
          qryWageStandardDetail.ParamByName('EMPLOYEE_NO').Value := memEmployeeEMPLOYEE_NO.Value;
          qryWageStandardDetail.ParamByName('WAGE_SCALE').Value := qryTemplateWAGE_SCALE.Value;
          qryWageStandardDetail.ParamByName('WAGE_LEVEL').Value := qryTemplateWAGE_LEVEL.Value;
          qryWageStandardDetail.ParamByName('WAGE_COEFF').Value := qryTemplateWAGE_COEFF.Value;
          qryWageStandardDetail.ParamByName('APPLY_MONTH').Value := qryTemplateAPPLY_MONTH.Value;
          qryWageStandardDetail.ParamByName('APPLY_YEAR').Value := qryTemplateAPPLY_YEAR.Value;
          qryWageStandardDetail.ParamByName('APPLY_PERIOD').Value := qryTemplateAPPLY_PERIOD.Value;
          dmMain.ExecuteSQL(qryWageStandardDetail);

          InsertMessage('   - Khởi tạo hệ số lương CBNN thành công !!!',count);
        except
          InsertMessage('   - Có lỗi trong quá trình khởi tạo hệ số lương CBNN ...',count);
          if not (memEmployee.State in [dsEdit]) then
            memEmployee.Edit;
            memEmployeeRESULT.Value:=1;
            OnError:=true;
...
procedure TfrmContractGenerate.dxDBPopupEdit5InitPopup(Sender: TObject);
begin
  inherited;
...
procedure TfrmContractGenerate.dxDBPopupEdit5CloseUp(Sender: TObject;
  var Text: WideString; var Accept: Boolean);
begin
  inherited;
  OnPopupCloseUp(Accept,frmPopupMain.qryWageStandard,qryTemplate,
...
procedure TfrmContractGenerate.acXoaViTriBoNhiemExecute(Sender: TObject);
begin
  inherited;
  if not (qryTemplate.State in [dsEdit,dsInsert] ) then
    qryTemplate.Edit;
  qryTemplatePOSITION_NO.Clear;
  qryTemplatePOSITION_NAME.Clear;
  qryTemplateASSIGNED_DATE.Clear;
  qryTemplateENDED_DATE.Clear;
  qryTemplate.Post;
end;

procedure TfrmContractGenerate.acXoaNgachLuongExecute(Sender: TObject);
begin
  inherited;
  if not (qryTemplate.State in [dsEdit,dsInsert] ) then
    qryTemplate.Edit;
  qryTemplateWAGE_SCALE.Clear;
  qryTemplateCAREER_NAME.Clear;
  qryTemplateWAGE_LEVEL.Clear;
  qryTemplateWAGE_COEFF.Clear;
  qryTemplateAPPLY_MONTH.Clear;
  qryTemplateAPPLY_YEAR.Clear;
  qryTemplateAPPLY_PERIOD.Clear;
  qryTemplate.Post;
end;

procedure TfrmContractGenerate.acXoaThamSoExecute(Sender: TObject);
begin
  inherited;
  qryExecute.SQL.Text:='delete from HR_CONTRACT_FORMAL WHERE TEMPLATE_NO=:TEMPLATE_NO; ';
  qryExecute.Prepare;
  qryExecute.ParamByName('TEMPLATE_NO').Value:=qryTemplateTEMPLATE_NO.Value;
  qryExecute.ExecSQL;
  if qryFormal.Active then
